...
juce::String EqProcessor::getBandID(size_t index)
{
    switch (index)
    {
    case 0:
...
const juce::String EqProcessor::getProgramName(int)
{
    return {};
}

void EqProcessor::changeProgramName(int, const juce::String &)
{
}
...
//==============================================================================
void EqProcessor::prepare(const juce::dsp::ProcessSpec &spec) noexcept
{
...
juce::String EqProcessor::getBandName(size_t index) const
{
    if (juce::isPositiveAndBelow(index, bands.size()))
...
juce::StringArray EqProcessor::getFilterTypeNames()
{
    return {
        TRANS("No Filter"),
        TRANS("High Pass"),
...
void EqProcessor::updateBand(const size_t index)
{
    if (sampleRate > 0)
    {
        ...
        case LowPass1st:
            ...
            break;
        case LowShelf:
            newCoefficients = juce::dsp::IIR::Coefficients<float>::makeLowShelf(
        ...
        case AllPass:
            newCoefficients =
                juce::dsp::IIR::Coefficients<float>::makeAllPass(sampleRate, bands[index].frequency, bands[index].quality);
            break;
        case AllPass1st:
        ...
        case Notch:
            newCoefficients =
                juce::dsp::IIR::Coefficients<float>::makeNotch(sampleRate, bands[index].frequency, bands[index].quality);
            break;
        case Peak:
            ...
            break;
        case HighShelf:
            newCoefficients = juce::dsp::IIR::Coefficients<float>::makeHighShelf(
                sampleRate, bands[index].frequency, bands[index].quality, bands[index].gain);
            break;
        case HighPass1st:
